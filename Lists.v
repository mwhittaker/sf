...
Fixpoint filter (f: nat -> bool) (xs: natlist) : natlist :=
  match xs with
  | [] => xs
  | x::xs =>
...
Fixpoint fold_left (f: nat -> nat -> nat) (a: nat) (xs: natlist) :=
  match xs with
  | [] => a
...
Fixpoint nonzeros (l:natlist) : natlist :=
  match l with
  | []   => []
  | x::l => (if negb (beq_nat x 0) then x::(nonzeros l) else nonzeros l)
  end.
...
Fixpoint oddmembers (l:natlist) : natlist :=
  filter (fun n => oddb n) l.

Example test_oddmembers:            oddmembers [0;1;0;2;3;0;0] = [1;3].
Proof. reflexivity. Qed.

Fixpoint countoddmembers (l:natlist) : nat :=
  fold_left (fun a n => a + (if oddb n then 1 else 0)) 0 l.
...
Fixpoint count (v:nat) (s:bag) : nat :=
  match s with
  | [] => 0
  | x::s => (count v s) + (if beq_nat x v then 1 else 0)
  end.
...
Fixpoint remove_one (v:nat) (s:bag) : bag :=
  match s with
  | [] => []
  | x::s => if beq_nat x v then s else x::(remove_one v s)
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  filter (fun x => negb (beq_nat x v)) s.
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with
  | [] => true
  | x::s1 => andb (negb (beq_nat (count x s2) 0)) (subset s1 (remove_one x s2))
  end.
...
Theorem bag_theorem :
  forall (x: nat) (b: bag), (count x b) + 1 = count x (add x b).
Proof.
  intros x b.
  induction b as [| v b' ].
  Case "b = []".
...
Theorem rev_snoc : forall (l: natlist) (x: nat), rev (snoc l x) = x::(rev l).
Proof.
  intros l x.
  induction l as [| y l'].
  Case "l = []".
...
Theorem snoc_app : forall (l1 l2 : natlist) (x: nat),
...
Fixpoint beq_natlist (xs ys : natlist) : bool :=
  match xs, ys with
  | [], []   => true
  | _::_, [] => false
  | [], _::_ => false
...
Theorem ble_plus_one : forall (n: nat), ble_nat n (n + 1) = true.
Proof.
  intros n.
  induction n as [| n'].
  Case "n = 0".
...
Theorem ble_plus : forall (n m: nat), ble_nat n (n + m) = true.
Proof.
  intros n m.
  induction n as [| n'].
  Case "n = 0".
